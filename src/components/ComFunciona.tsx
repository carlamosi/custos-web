      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        ...
        <div className="mt-16 md:mt-24 relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-10 md:gap-12 relative">
            {steps.map((step, i) => {
              const { ref: sRef, isVisible: sVis } = useScrollReveal();
              return (
                <motion.div
                  key={i}
                  ref={sRef}
                  ...
                  initial={{ opacity: 0, y: 30 }}
                  animate={sVis ? { opacity: 1, y: 0 } : {}}
                  transition={{ duration: 0.6, delay: (i + 1) * 0.1 }}
                >
                  <div className={`w-20 h-20 md:w-24 md:h-24 rounded-full flex items-center justify-center shadow-sm mb-6 z-10 relative ${step.dark ? "bg-primary shadow-md" : "bg-white border border-foreground/5"}`}>
                    <span className={`absolute -top-1 -right-1 md:-top-2 md:-right-2 w-6 h-6 md:w-8 md:h-8 bg-sage text-foreground text-xs font-medium rounded-full flex items-center justify-center border-2 ${step.dark ? "border-primary" : "border-white"}`}>
                      {i + 1}
                    </span>
                    {step.icon}
                  </div>
